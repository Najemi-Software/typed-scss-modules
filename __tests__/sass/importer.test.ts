...
import { aliasImporter, aliasResolver, customImporters, type SASSImporter } from "../../lib/sass/importer.js";
...
const endSegments = (url: URL | Promise<URL | null> | null, segmentsN = 1): Promise<string> | string => {
...
describe("#aliasImporter", () => {
    it("should create an importer to replace aliases and otherwise return null", () => {
        ...
        expect(endSegments(importer.findFileUrl("input", contextStub))).toEqual("output");
        expect(endSegments(importer.findFileUrl("~alias", contextStub))).toEqual("node_modules");
    ...
    it("should create an importer to replace alias prefixes and otherwise return null", () => {
        ...
        expect(endSegments(importer.findFileUrl("abc-123", contextStub))).toEqual("def-123");
        expect(endSegments(importer.findFileUrl("~package", contextStub), 2)).toEqual("node_modules/package");
        expect(importer.findFileUrl("output~", contextStub)).toBeNull();
